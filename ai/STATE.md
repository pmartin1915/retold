...
- [ ] R1 Data model (PLAN §6.1) -- **implemented by Kimi 2026-09-22 (`518a304`, branch `r1-impl`, draft PR #1), NOT compiled: CI is blocked on GitHub billing** (same account-wide block as StoryCue: "recent account payments have failed or your spending limit needs to be increased"; operator-only). Spec `docs/R1-DATAMODEL-SPEC.md` (`84f1f6e`), Kimi spec review `docs/reviews/R1-SPEC-REVIEW-2026-09-22.md`. All 32 tests present by name; greps clean; boss read of `Confirmable.swift`/`Entities.swift` found the Rule-1 invariants held. Sol Rule-1 audit adjudicated: a real hole (a model-authored episode title could be confirmed) fixed in `f590446` on `r1-impl`; record `docs/reviews/R1-SOL-AUDIT-2026-09-22.md` (on the branch); 4 findings deferred to R2/R7 in `ai/IDEAS.md` (on the branch) -- the R2 spec must make `VerifiedSpan` verifier-only. Session handoff: `../storycue/docs/HANDOFF-2026-09-22-s1-s2a-r1.md`. When CI runs, read it as the spike: `testDetailStoredProvenanceSurvivesReopen` passing while `testConfirmableIntSurvivesReopen` fails = genericity -> pre-registered fallback (concrete `ConfirmableInt`/`ConfirmableString`) via a follow-up spec.
